...
//! `can_use_different_words_with_the_same_name` test, it has its own problem:
//! it can be attacked to cause an OOM condition in an arbitrary machine without
...
// From this point forward, we define the custom `GLOBAL_ALLOCATOR` used in the
// test above.

use std::{
	alloc::{GlobalAlloc, Layout, System as SystemAllocator},
