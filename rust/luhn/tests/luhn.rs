...
fn numbers_work() {
	assert_eq!(
		to_numbers("4539 3195 0343 6467".chars()),
		Some(vec![4, 5, 3, 9, 3, 1, 9, 5, 0, 3, 4, 3, 6, 4, 6, 7])
	)
}
...
fn doubling_works() {
	assert_eq!(
		to_numbers("4539 3195 0343 6467".chars())
			.map(|numbers| double(numbers.into_iter()).collect_vec()),
		to_numbers("8569 6195 0383 3437".chars())
	);
}
...
#[test]
// #[ignore]
/// non-numeric, non-space char in the middle with a sum that's divisible by 10
/// isn't allowed
...
#[test]
// #[ignore]
/// unicode numeric characters are not allowed in an otherwise valid number
