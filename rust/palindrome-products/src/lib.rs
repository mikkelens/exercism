...
impl Palindrome {
	...
	/// Find out if a number is a palindrome by reversing the bytes of its
	/// string representation and comparing it to the original. Furthermore, we
	/// only need to compare each half of the string bytes, since a palindrome
	...
	fn is_palindrome(value: u64) -> bool {
		...
		first.eq(second) // if first half is equal to second half in reverse, it is a
		         // palindrome
	}
}
...
pub fn palindrome_products(min: u64, max: u64) -> Option<(Palindrome, Palindrome)> {
	...
	/// In order to avoid allocating every combination at once, we instead
	/// create an iterator that does this incrementally. By giving this function
	/// the right input we can find the min or max palindrome in a range-based
	/// iterator, without having to explore every possible combination.
	fn best_palindrome_by_order(
		iter_oriented: impl Iterator<Item = u64> + Clone, /* ranges are only reversible as
		                                                   * iterators */
		ord_target: Ordering
	) -> Option<Palindrome> {
		...
		iter_oriented
			...
			.fold_while(None, |best_so_far, (a, b)| {
				...
				{
					// cannot yield a product with superior ordering from this point, optimal next
					// is `a * ideal`
					...
					FoldWhile::Continue(
							...
							superior_found.then(|| Palindrome::new(product)).flatten()
						{
							...
							best_so_far // new product does not have superior ordering OR
							 // not a palindrome
						}
					)
				}
	...
	// if min and max are the same, we have "wasted time" looking for the same value
	// twice, but this is insignificant because this may only happen on small
	// inputs, and cannot generally be predicted

	min.zip(max) // None-precedence of Option-zip is irrelevant because min and max
